...
func _ready():
	...
	if origin=="Enemy":$label.modulate=Color("#A30015")#Color("#FFAAAA")
	self.modulate.a=0
	$twnScale.interpolate_property(self,"scale.x",10,self.scale.x,duration,Tween.TRANS_BACK,Tween.EASE_OUT)
